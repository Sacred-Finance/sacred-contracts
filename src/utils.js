...
function getExtWithdrawRewardArgsHash({ fee, recipient, relayer, encryptedAccount }) {
  const encodedData = web3.eth.abi.encodeParameters(
    [WithdrawExtData],
    [
      {
...
function getExtWithdrawAssetArgsHash({ recipient, relayer, fee, refund }) {
  const encodedData = web3.eth.abi.encodeParameters(
    [WithdrawAssetExtData],
    [
      {
