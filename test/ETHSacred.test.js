...
function snarkVerify(proof) {
  ...
  const verification_key = unstringifyBigInts2(
    require('../build/circuits/WithdrawAsset_verification_key.json'),
  )
...
contract('ETHSacred', (accounts) => {
  ...
  describe('#withdraw', () => {
    ...
    it('fee should be less or equal transfer value', async () => {
      ...
      const extData = { recipient, relayer: operator, fee: largeFee, refund }
      const extDataHash = getExtWithdrawAssetArgsHash(extData)
      const input = stringifyBigInts({
        root,
        nullifierHash: pedersenHash(deposit.nullifier.leInt2Buff(31)),
    ...
    it('should reject with non zero refund', async () => {
      ...
      const extData = { recipient, relayer: operator, fee, refund: bigInt(1) }
      const extDataHash = getExtWithdrawAssetArgsHash(extData)
