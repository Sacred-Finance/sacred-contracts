...
async function registerNote(note, sacredTrees) {
...
contract('Miner', (accounts) => {
  ...
  let rewardSwap
  let sacredTrees
  const sacredAddr = '0x3535249DFBb73e21c2aCDC6e42796d920A0379b7'
  ...
  before(async () => {
    ...
    const withdrawRewardVerifier = await WithdrawRewardVerifier.new()
    ...
    for (const note of notes) {
      const { depositLeaf, withdrawalLeaf } = await registerNote(note, sacredTrees)
      depositData.push(depositLeaf)
      withdrawalData.push(withdrawalLeaf)
    }

    // await sacredTrees.updateRoots(depositData, withdrawalData)
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      ...
      const rateFromContract = await miner.rates(sacredAddr)
  ...
  describe('#Note.fromString()', () => {
    it('should work', () => {
      const note = Note.fromString(
        'sacred-eth-1-1-0x3a1f1e0e10b22b15ed8208bc810dd5564d564fd7930874db4d7d58870deb72978fb5fccae2f1554cac71e2cff85f9c8908295647adf2b443a4dd93635d8d',
  ...
  describe('#reward', () => {
    ...
    it('should send fee to relayer', async () => {
      ...
      await timeReset()
      let expectedFeeInSacred = await rewardSwap.getExpectedReturn(fee)
      let relayerBalanceBefore = await sacred.balanceOf(relayer)
      await miner.reward(claim.proof, claim.args)
      let relayerBalanceAfter = await sacred.balanceOf(relayer)
      relayerBalanceAfter.should.be.eq.BN(relayerBalanceBefore.add(expectedFeeInSacred))
      ...
      expectedFeeInSacred = await rewardSwap.getExpectedReturn(amount.add(fee))
      expectedFeeInSacred = expectedFeeInSacred.sub(expectedAmountInSacred)

      relayerBalanceBefore = await sacred.balanceOf(relayer)
      const recipientBalanceBefore = await sacred.balanceOf(recipient)
      await miner.withdraw(withdrawal.proof, withdrawal.args)
      const recipientBalanceAfter = await sacred.balanceOf(recipient)
      relayerBalanceAfter = await sacred.balanceOf(relayer)

      recipientBalanceAfter.should.be.eq.BN(recipientBalanceBefore.add(expectedAmountInSacred))
      relayerBalanceAfter.sub(relayerBalanceBefore).sub(expectedFeeInSacred).should.be.lt.BN(delta)
    })
    ...
    it('should work with outdated deposit or withdrawal merkle root', async () => {
      ...
      const note4Leaves = await registerNote(note4, sacredTrees)
      // await sacredTrees.updateRoots([note4Leaves.depositLeaf], [note4Leaves.withdrawalLeaf])

      const claim2 = await controller.reward({ account: new Account(), note: note4, publicKey })

      for (let i = 0; i < 9; i++) {
        const note0Leaves = await registerNote(note0, sacredTrees)
        // await sacredTrees.updateRoots([note0Leaves.depositLeaf], [note0Leaves.withdrawalLeaf])
      }
      ...
      const note5Leaves = await registerNote(note5, sacredTrees)
      // await sacredTrees.updateRoots([note5Leaves.depositLeaf], [note5Leaves.withdrawalLeaf])
  ...
  describe('#withdraw', () => {
    ...
    it('should reject for unfair amount', async () => {
      ...
      await timeReset()
      const balanceBefore = await sacred.balanceOf(recipient)
      const expectedAmountInSacred = await rewardSwap.getExpectedReturn(amountToWithdraw)
      await miner.withdraw(proof, args)
      const balanceAfter = await sacred.balanceOf(recipient)
      balanceAfter.should.be.eq.BN(balanceBefore.add(expectedAmountInSacred))
    })
    ...
    it('should reject for malformed relayer and recipient address and fee', async () => {
      ...
      const extDataHash = getExtWithdrawRewardArgsHash({
        fee: fakeFee,
  ...
  describe('#setRates', () => {
    it('should reject for invalid rates', async () => {
      ...
      await miner
        .setRates([{ instance: sacredAddr, value: bigNum.toString() }], { from: governance })
  ...
  describe('#setVerifiers', () => {
    it('onlyGovernance can set new verifiers', async () => {
      ...
      const withdrawRewardVerifier = await miner.withdrawRewardVerifier()
      withdrawRewardVerifier.should.be.equal(verifiers[1])
